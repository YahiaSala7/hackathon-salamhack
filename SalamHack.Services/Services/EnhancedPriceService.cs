...
namespace SalamHack.Services.Services
{
    public class EnhancedPriceService : IPriceComparisonService, IPriceSearchService
    {
        ...
        // ======== تنفيذ واجهة IPriceSearchService ========
        ...
        public async Task<List<NearbyStoreDto>> FindNearbyStoresAsync(int projectId, int radiusKm = 20)
        {
            ...
            try
            {
                // استدعاء الدالة الخاصة لاسترجاع المتاجر القريبة
                var stores = await CallSearchNearbyStoresAsync(project.Location, radiusKm);
        ...
        public async Task<List<FurnitureAvailabilityDto>> FindFurnitureAvailabilityAsync(int furnitureId, int projectId, int radiusKm = 20)
        {
            ...
            try
            {
                // استدعاء الدالة الخاصة لاسترجاع توفر الأثاث
                var availabilityList = await CallSearchFurnitureAvailabilityAsync(furnitureId, project.Location, radiusKm);
                var validatedAvailability = await ProcessAvailabilityData(availabilityList, furnitureId, project.Location);
        ...
        public async Task<List<PriceComparisonDto>> SearchPriceComparisonsAsync(string furnitureName, string category)
        {
            ...
            try
            {
                // استدعاء الدالة الخاصة لاسترجاع المقارنات السعرية
                var comparisons = await CallGetPriceComparisonsAsync(furnitureName, category);
        ...
        // ======== تنفيذ واجهة IPriceComparisonService ========
        ...
        // ======== أساليب مساعدة خاصة لتحل محل واجهة IExternalPriceApiClient ========

        // دالة لمحاكاة استدعاء API للمتاجر القريبة
        private async Task<List<NearbyStoreDto>> CallSearchNearbyStoresAsync(string location, int radiusKm, int? furnitureId = null)
        {
            // هنا يمكنك وضع منطق الاتصال بالخدمة الخارجية
            await Task.CompletedTask;
            // استبدل السطر التالي بمنطقك الفعلي
            return new List<NearbyStoreDto>();
        }

        // دالة لمحاكاة استدعاء API لتوفر الأثاث
        private async Task<List<FurnitureAvailabilityDto>> CallSearchFurnitureAvailabilityAsync(int furnitureId, string location, int radiusKm)
        {
            await Task.CompletedTask;
            // استبدل السطر التالي بمنطقك الفعلي
            return new List<FurnitureAvailabilityDto>();
        }

        // دالة لمحاكاة استدعاء API للمقارنات السعرية
        private async Task<List<PriceComparisonDto>> CallGetPriceComparisonsAsync(string furnitureName, string category)
        {
            await Task.CompletedTask;
            // استبدل السطر التالي بمنطقك الفعلي
        ...
        // دالة تجمع بيانات الأسعار من الدوال الخاصة
        private async Task<(List<PriceComparisonDto> OnlinePrices, List<NearbyStoreDto> NearbyStores)> GetPriceDataAsync(Furniture furniture, Project project)
        {
            var onlinePricesTask = CallGetPriceComparisonsAsync(furniture.Name, furniture.Category);
            var nearbyStoresTask = CallSearchNearbyStoresAsync(project.Location, 20, furniture.FurnitureId);

            await Task.WhenAll(onlinePricesTask, nearbyStoresTask);
            return (await onlinePricesTask, await nearbyStoresTask);
        }

        // باقي الأساليب الخاصة بمعالجة البيانات وتخزين المقارنات تظل كما هي:
        ...
        private async Task<List<PriceComparison>> ProcessNearbyStores(int furnitureId, List<NearbyStoreDto> nearbyStores, string projectLocation)
        {
            var comparisons = new List<PriceComparison>();
            ...
            int maxDistance)
        {
            ...
            foreach (var store in stores)
            {
                try
                {
                    double distance = await _geocodingService.CalculateDistanceAsync(projectLocation, store.Address);
            ...
            string projectLocation)
        {
            ...
            foreach (var item in availabilityList)
            {
                try
                {
                    item.Distance = Math.Round(await _geocodingService.CalculateDistanceAsync(projectLocation, item.StoreAddress), 1);
                    if (item.Distance <= 20)
                    {
        ...
        private async Task UpdatePriceComparison(int furnitureId, FurnitureAvailabilityDto item)
        {
            try
            {
                var existing = await _priceComparisonRepository.GetByFurnitureIdAndStoreAsync(furnitureId, item.StoreName);
                if (existing == null)
                {
