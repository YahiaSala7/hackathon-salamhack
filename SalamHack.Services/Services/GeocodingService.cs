...
namespace SalamHack.Services.Services
{
    public class GeocodingService : IGeocodingService
    {
        ...
        public GeocodingService(HttpClient httpClient, string apiKey, IMemoryCache cache)
        {
        ...
        public async Task<(double latitude, double longitude)> GetCoordinatesAsync(string address)
        {
            ...
            var requestUrl = $"{BaseUrl}?address={Uri.EscapeDataString(address)}&key={_apiKey}";
        ...
        public async Task<string> GetAddressAsync(double latitude, double longitude)
        {
            ...
            // Cache miss - call the API
            var requestUrl = $"{BaseUrl}?latlng={latitude},{longitude}&key={_apiKey}";
            var response = await _httpClient.GetStringAsync(requestUrl);
            var geocodingResponse = JsonSerializer.Deserialize<GeocodeResponse>(response);
