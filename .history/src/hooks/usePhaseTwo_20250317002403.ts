...
export const usePhaseTwo = (): UsePhaseTwoReturn => {
  ...
  const { data, isLoading, error } = useQuery<PhaseTwoData>({
    queryKey: ["phaseTwo"],
    queryFn: async () => {
    ...
    enabled: isPhaseOneCompleted, // Only fetch if phase one is completed
  });
  ...
  return {
    data: isPhaseOneCompleted ? (data as PhaseTwoData) : placeholderData,
    isLoading,
