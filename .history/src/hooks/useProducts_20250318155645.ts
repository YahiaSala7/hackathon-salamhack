...
export const useProducts = (formData: FormData | null) => {
  const {
    ...
    queryFn: async () => {
      ...
      const response = await productsApi.getProducts(formData);
      if (!response.data) {
  ...
  // Use sample data when formData is not available
  const products = formData ? productsData || [] : SAMPLE_PRODUCTS;
