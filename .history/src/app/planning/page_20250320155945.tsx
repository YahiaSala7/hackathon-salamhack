...
// Remove the useCombinedData import since we'll use the response directly
...
import { CombinedResponse } from "@/services/combinedApi"; // Import the response type
...
function Planning() {
  ...
  // Replace useCombinedData with direct state management
  const [combinedData, setCombinedData] = useState<CombinedResponse | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);

  const handleFormSubmit = useCallback((data: FormData, responseData: CombinedResponse) => {
    setFormData(data);
    setCombinedData(responseData); // Store the response data
