...
function PhaseOne() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      // Show loading spinner in button for 2 seconds
      ...
      submitHome(data, {
        onSuccess: () => {
          ...
          // Only show loading overlay after successful submission
          setShowLoading(true);
          ...
          const progressInterval = setInterval(() => {
            setProgress((prev) => {
              if (prev >= 100) {
                ...
                // Navigate to next phase after progress reaches 100%
                setTimeout(() => {
        ...
        onError: (error) => {
          ...
          // Don't show loading overlay on error
        ...
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-2">
          ...
          <form
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-full p-4 mt-2 text-xl text-white bg-button rounded-[10px] font-bold hover:bg-button/90 disabled:bg-button/50 disabled:cursor-not-allowed transition-colors shadow">
              {isPending ? (
