    ...
    <div className={`relative ${className}`}>
      <div
        className={`flex items-center justify-between w-full p-4 rounded-[4px] cursor-pointer ${
          error ? "bg-error/5 border border-error" : "bg-background"
        }`}
...
>(({ error, ...props }, ref) => (
  <input
    ...
    className={`w-full p-4 rounded-[4px] focus:outline-none ${
      error
        ? "bg-error/5 border border-error text-error placeholder-error/80 font-medium"
        : "bg-background placeholder:text-text/60"
    }`}
...
const CombinedField = ({
  ...
  <div
    className={`relative flex rounded-[4px] ${
