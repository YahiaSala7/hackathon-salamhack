...
import { BudgetEntry } from "@/types/product";
...
export const BudgetChart: FC<BudgetChartProps> = ({ budgetDistribution }) => {
  // Memoize budget distribution data for pie chart
  const data = useMemo(() => budgetDistribution, [budgetDistribution]);
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                ...
                labelLine={false}>
                {data.map((entry: BudgetEntry, index: number) => (
                  <Cell
                    key={`cell-${index}`}
