...
const PhaseTwo: FC<PhaseTwoProps> = ({
    ...
    if (isFormSubmitted) {
      ...
      if (storedData) {
        try {
          ...
          console.log("Retrieved submission data in PhaseTwo:", parsedData);
  ...
  const displayData = useMemo(() => {
    if (isFormSubmitted && localData) {
      return {
        budgetDistribution: localData.budgetDistribution || [],
        recommendations: localData.recommendations || {},
      };
    }
    return fakeRecommendationsData;
  }, [isFormSubmitted, localData]);
