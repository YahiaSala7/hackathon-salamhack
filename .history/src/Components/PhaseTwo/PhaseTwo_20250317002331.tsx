...
const PhaseTwo: FC = () => {
  const { data, isLoading, error, isPreviewMode } = usePhaseTwo();
  ...
  return (
    <div className="px-4 py-6 mx-auto max-w-7xl">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                ...
                {data.budgetData.map((entry: BudgetItem, index: number) => (
                  <Cell
                    key={`cell-${index}`}
      ...
      <div className="space-y-16">
        {Object.entries(data.recommendationsByCategory).map(
          ([category, items]: [string, RecommendationItem[]]) => (
