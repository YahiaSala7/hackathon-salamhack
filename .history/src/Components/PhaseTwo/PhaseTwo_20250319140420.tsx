    ...
    () => ({
      ...
      beforeChange: () => {
        if (sliderRef.current) {
          const currentSlide =
            (sliderRef.current as any).slickCurrentSlide() || 0;
          const visible = Array.from({ length: 3 }, (_, i) => currentSlide + i);
                  ...
                  {items.map((item: RecommendationItem, index: number) => (
                    <div key={item.id} className="p-2 sm:p-4 h-full">
                      <div className="h-full bg-background rounded-lg shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden flex flex-col">
                        <div className="relative pt-[75%] md:pt-[66.67%] overflow-hidden bg-gray-100">
                          <Image
                            src={item.image}
                            alt={item.title}
                            ...
                            loading={
                              visibleSlides.includes(index) ? "eager" : "lazy"
                            }
                            priority={index < 3}
                            placeholder="blur"
