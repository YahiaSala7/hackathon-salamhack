...
const StarRating: React.FC<{ rating: number; size?: "small" | "medium" }> = ({
  ...
  const stars = Array.from({ length: 5 }, (_, index) => {
    ...
    return (
      <span key={index} className="text-heading">
        {filled ? (
...
const PhaseThree: React.FC<PhaseThreeProps> = ({
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      {/* Filter Toggle Button (Above cards) */}
      <div className="lg:hidden mb-4">
        <button
          onClick={() => setIsFilterOpen(!isFilterOpen)}
          className="w-full flex items-center justify-between px-4 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg shadow-lg hover:from-blue-600 hover:to-blue-700 transition-all duration-200">
      ...
      <div
        ...
        <div
          className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white/95 backdrop-blur-md rounded-lg shadow-xl transition-all duration-300 ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Left Side - Filter Panel and Map (Only visible on large screens) */}
        <div className="hidden lg:block lg:col-span-1 flex flex-col space-y-1">
          {/* Filter Panel */}
                ...
                <Marker
                  ...
                  <Popup>
                    <div className="p-2">
                      ...
                      <p className="text-sm text-text mb-2">{product.title}</p>
        ...
        <div className="lg:col-span-3">
          {/* Map Section (Visible on small and medium screens) */}
          <div className="lg:hidden mb-4">
              ...
              <div className="overflow-x-auto flex-grow">
                <table className="w-full h-full">
                  <thead className="bg-gray-50 border-b">
                    <tr>
                      <th className="px-2 py-4 text-left text-sm font-medium text-text w-[20%]">
                        Title
                      </th>
                      <th className="px-2 py-4 text-left text-sm font-medium text-text w-[15%]">
                        Category
                      </th>
                      <th className="px-2 py-4 text-left text-sm font-medium text-text w-[10%]">
                        Image
                      </th>
                      <th className="px-2 py-4 text-left text-sm font-medium text-text w-[10%]">
                        Price
                  ...
                  <tbody className="divide-y-4 divide-background">
                    {currentItems.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        <td className="px-2 py-4 text-sm text-text truncate">
                          {item.title}
                        </td>
                        <td className="px-2 py-4 text-sm text-text">
                          <div className="flex items-center gap-2">
                            <div
                        ...
                        <td className="px-2 py-4 text-sm text-heading">
                          ${item.price}
                        </td>
                        <td className="px-2 py-4 text-sm text-text truncate">
                          {item.store.name}
