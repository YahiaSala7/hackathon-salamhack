      ...
      <div className="overflow-x-auto flex-grow">
        <table className="w-full h-full table-fixed">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="px-6 py-4 text-left text-sm font-medium text-gray-500 w-[20%]">
                Title
              </th>
              <th className="px-6 py-4 text-left text-sm font-medium text-gray-500 w-[15%]">
                Category
              </th>
              <th className="px-6 py-4 text-left text-sm font-medium text-gray-500 w-[10%]">
                Image
              </th>
              <th className="px-6 py-4 text-left text-sm font-medium text-gray-500 w-[10%]">
                Price
          ...
          <tbody
            className={`divide-y divide-gray-200 ${
              currentItems.length < itemsPerPage ? "" : "h-full"
            }`}>
            {currentItems.map((item) => (
              <tr
                key={item.id}
                className={`hover:bg-gray-50 ${
                  currentItems.length < itemsPerPage ? "" : "h-[1fr]"
                }`}>
      ...
      {currentItems.map((item) => (
        <div
          ...
          <div className="flex items-start justify-between mb-3">
            ...
            <span className="text-lg font-bold text-heading ml-4">
              ${item.price}
            </span>
          </div>
          ...
          <div className="mb-3">
            <a
              href="#"
              className="text-blue-300 hover:text-blue-400 text-sm block truncate">
              {item.image}
            </a>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <a
                href="#"
                className="text-blue-300 hover:text-blue-400 text-sm truncate ml-2">
                {item.location}
              </a>
            </div>
    ...
    <div className="min-h-screen p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          {/* View Toggle Button - Only visible on large screens */}
          <div className="hidden lg:flex justify-end">
            <button
              onClick={() => setIsTableView(!isTableView)}
              className="px-4 py-2 bg-white rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
              Switch to {isTableView ? "Card" : "Table"} View
            </button>
          </div>

          {/* Table View - Only visible on large screens when toggled */}
          <div
            className={`hidden lg:block ${isTableView ? "block" : "hidden"}`}>
            <TableView />
          </div>

          {/* Card View - Always visible on small screens, toggleable on large screens */}
          <div className={`block lg:${isTableView ? "hidden" : "block"}`}>
            <CardView />
          </div>

          {/* Pagination */}
          <div className="bg-white rounded-lg shadow-sm px-6 py-4 mt-4">
            <div className="flex items-center justify-between">
