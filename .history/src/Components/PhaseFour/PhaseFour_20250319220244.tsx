...
function PhaseFour({ formData, recommendations }: PhaseFourProps) {
  ...
  const generatePrompt = (roomCategory: Category) => {
    ...
    const prompt = `Generate an image of a ${roomCategory} interior design for a home in ${formData.location}. The home has an area of ${formData.area} ${formData.areaUnit}. Consider the following:
