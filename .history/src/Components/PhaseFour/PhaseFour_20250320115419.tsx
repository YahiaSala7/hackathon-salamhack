...
function PhaseFour({ formData, recommendations }: PhaseFourProps) {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      try {
        ...
        console.log("Retrieved submission data in PhaseFour:", parsedData);
  ...
  const generatePrompt = (roomCategory: Category) => {
    // Use localData if available, otherwise fallback to props or defaults
    ...
    // Use recommendations from localData or props
    const roomItems =
      localData?.recommendations?.[roomCategory] ||
      recommendations?.[roomCategory] ||
      fakeRecommendationsData.recommendations[roomCategory] ||
      [];

    const itemsList = roomItems.map((item: any) => item.title).join(", ");
    ...
    <div className="min-h-screen px-4  py-6 m-auto flex flex-col">
      <div className="text-left mb-12">
        ...
        <p className="text-md md:text-lg text-gray-600 max-w-3xl mx-auto text-center">
          Visualize your dream rooms with AI-generated interior designs. Select
          a room type below to generate a personalized design based on your
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {roomCategories.map((room) => (
          <button
            ...
            disabled={isPending}
            className={`w-full lg:text-lg md:text-md text-sm font-semibold text-white flex justify-center items-center gap-2 px-3 md:px-5  py-3 rounded-md cursor-pointer bg-heading hover:bg-[#1A1F2C] 
             `}>
            {room}
          </button>
        ))}
