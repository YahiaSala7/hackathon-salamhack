...
${JSON.stringify(roomItems, null, 2)}`;
  ...
  return (
    <div className="min-h-screen px-4  py-6 m-auto flex flex-col">
      ...
      {/* Image Display Area */}
      <div className="mb-8">
        <div className=" w-full bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center relative">
          {isPending ? (
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {roomCategories.map((room) => (
          <button
            ...
            disabled={isPending}
            className={`w-full text-lg font-semibold text-white flex justify-center gap-2 px-5 py-3 rounded-md cursor-pointer bg-heading hover:bg-[#1A1F2C] 
             `}>
            {room}
          </button>
        ))}
